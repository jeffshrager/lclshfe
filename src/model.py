...
from src.library.functions.func import config_print, create_experiment_figure, \
    experiment_is_not_over, experiment_stats, generate_layout, generate_table, goal_agenda_plan
...
def model(config:Config) -> AMI:
    ...
    with Live(generate_layout(context), refresh_per_second=60, screen=True) as live:
        while experiment_is_not_over(context):
            ...
            live.update(generate_layout(context))
            # print(goal_agenda_plan(context) if context['settings']['display'] else 'running')
            # print(str(context.messages) if context['settings']['display'] else 'running')

            if run_started_tracker:
                # TODO: add this to config
                print('[cyan]Press Enter to continue')
                input()
                # sleep(5.0)
        ...
        "[green]Finished" if context['settings']['display'] else '-')
